...
public class CraftBet_000_CasinoSameNameCopyGames_Test extends BaseTest {
    ...
    public boolean getALLGamesAPICheckCopyGames()
        ...
        if (errSet.size() == 0) {
            basePage.writeInExelSet(errSet, "/src/test/java/CraftBet_001_APICasinoGamesBrokenData/" + readConfig.partnerConfigNum() + getGamesPartnerName + "BrokenData.xlsx", "CopyGames");
            isPassed = true;
        } else {
            basePage.writeInExelSet(errSet, "/src/test/java/CraftBet_001_APICasinoGamesBrokenData/" + readConfig.partnerConfigNum() + getGamesPartnerName + "BrokenData.xlsx", "CopyGames");
            isPassed = false;
